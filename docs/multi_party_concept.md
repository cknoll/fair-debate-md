...
- **Key-Bedeutung (Q3b, Modell X):** Der role-token im Key **identifiziert die Partei**, nicht nur
  einen Geschwister-Index. Maximale Transparenz: aus dem Key ist ablesbar, von welcher Partei
  ein Statement stammt.
- **role-token-Geltungsbereich (Klarstellung):** role-tokens gelten **pro Debatte**, nicht global.
  Ein User kann in Debatte 1 role-token `b` und in Debatte 2 role-token `f` haben. Für N globale User
  braucht es **nicht** N role-tokens, sondern nur so viele, wie *in einer einzelnen Debatte*
  teilnehmen. Erschöpfung tritt erst bei >26 Parteien *in derselben Debatte* ein → dann
  mehrstellige role-tokens (`aa`, `ab`, …).
- **Sortierung (Q4):** v1 chronologisch, älteste zuerst. Später umstellbar (Sortier-Option).
- **Repo-/Git-Modell (Q6a):** v1 weiterhin **ein** Repo pro Debatte, Unterordner pro role-token
...
- **Antworten pro Partei/Segment (Q7a + ii):** Eine Partei hat **höchstens eine** direkte
  Antwort pro Segment (editierbar), da `Segment + role-token` eindeutig ist. Eine Partei **darf**
...
Ein Key kodiert den **Pfad im Debatten-Baum**; jeder role-token ist eine Partei-Kennung,
...
Eigenschaften:
- Der **Wurzel-role-token ist immer `a`** (Eröffner der Debatte).
- Der Kind-Key ergibt sich aus **Eltern-Segment + role-token des Antwortenden** —
  *nicht* mehr aus einem `a↔b`-Flip.
- `Segment + role-token` ist eindeutig → eine Partei hat max. eine direkte Antwort pro Segment.
- role-token-Vergabe: bei **erster Teilnahme** einer Partei in einer Debatte wird der nächste
  freie role-token vergeben (`a`, `b`, …, `z`, `aa`, `ab`, …).
...
class DebateParticipant(models.Model):
    ...
    role_token = CharField    # "a", "b", ..., "aa", ... ; pro Debatte eindeutig
    repo   = ForeignKey(Repo, null=True)   # optional, für Repo-Link in der UI
    # unique_together: (debate, user) und (debate, role_token)
```

- `Debate.user_a/user_b/repo_a/repo_b` entfallen.
- `Debate.get_user_role(user)` → gibt den role-token der Partei zurück (oder None).
- `Debate.get_for_user(...)` → Query über `DebateParticipant` statt über user_a/user_b.
- role-token-Allokator: Hilfsfunktion "nächster freier role-token in dieser Debatte".
...
- `core.py:357` `key_regex = r"[ab]\d+"` → `r"[a-z]+\d+"` (mehrstellige role-tokens erlaubt).
...
- `core.py:539` `get_next_turn_key(segment_key)` (a↔b-Flip) **entfällt** bzw. wird ersetzt:
  Der nächste role-token ergibt sich aus dem **Antwortenden**, nicht aus dem Eltern-Key.
- `core.py:531` `get_contribution_key(segment_key)` → bekommt zusätzlich den
  **Antwortenden-role-token**: `get_contribution_key(segment_key, answering_token)`.
...
- `core.py:410-411` `dir_a`, `dir_b` → dynamische Verzeichnisse pro role-token (`a/`, `b/`, `c/`…).
  `load_dir` (`core.py:437-438`) globbt entsprechend alle role-token-Verzeichnisse.
- `core.py:464` `self.root_mdp = self.tree["a"]` bleibt (Wurzel ist role-token `a`).
- `core.py:611-628` `write_ctb_to_file`: `author_role = ctb_key[-1]` → letzter **role-token**
  (nicht zwingend einstellig); `assert in [a,b]` entfällt; Zielverzeichnis = role-token.
- `core.py:607` / `repo_handling.py:59` `get_author(debate_key, author_role)`:
  Rollen-Argument wird zum role-token verallgemeinert.
...
- `views.py:88` `NewDebateView.post`: statt `user_a=request.user` einen
  `DebateParticipant`-Eintrag mit role-token `a` anlegen.
- `views.py:313-314` `contribution_mode = contribution_key[-1]; assert in (a,b)`:
  Mode = letzter role-token; Assertion entfernen/verallgemeinern.
- `views.py:312` Kind-Key künftig aus Segment + role-token des aktuellen Users.
- `views.py:355` `_ensure_suitable_user_role`: Logik "wer Rolle b greifen darf" →
  "User ist bereits Partei (nutzt seinen role-token)" **oder** "neuer User → nächster freier
  role-token wird vergeben". Eine Partei darf ihr eigenes Segment beantworten (Q7-ii).
- `views.py:405-423` `render_result_from_html`: `user_b == "__undefined__"`-Sonderfall →
  Teilnehmer-Liste (role-token → User/Repo) in den Context geben.
...
- `static/core.js:86-87,165,213,433-438`: Antwort-Routing und Hinweise sind an die Rollen
  `a`/`b` und `user_b` gebunden → auf beliebige role-tokens + Teilnehmer-Liste umstellen.
...
- **Einzig Neues:** Ein Segment kann **mehrere** direkte Antworten haben → diese erscheinen als
  ...
  (role-token + Username, Klartext-Identität aus Teilnehmer-Liste). Die Verschachtelungstiefe
...
ersten beiden role-tokens. Daten-Migration:
- `Debate.user_a` → `DebateParticipant(role_token="a")`
- `Debate.user_b` → `DebateParticipant(role_token="b")` (falls gesetzt)
